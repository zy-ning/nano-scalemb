...
code lives in nano_scalemb directory and is imported from here.
...
def _conversation_messages_for_audit(conversation):
    messages = []
...
def _append_audit_record(audit_file_path, record):
...
def run_generative_eval(
    ...
    for i in range(ddp_rank, num_problems, ddp_world_size):
        ...
        outcome_details = [
            task_object.evaluate_with_details(conversation, completion)
            for completion in completions
        ]
        outcomes = [bool(details["passed"]) for details in outcome_details]
        passed = any(outcomes)
        ...
        audit_record = {
            ...
            "completions": [
                ...
                for sample_idx, (completion, details) in enumerate(
                    zip(completions, outcome_details)
                )
            ],
        }
        _append_audit_record(audit_file_path, audit_record)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Distributed process-group timeout in minutes for chat evaluation",
    )
    parser.add_argument(
        "--audit-dir",
        type=str,
        default="",
        help="Optional directory to write per-example JSONL audit logs",
    )
    ...
    dist_timeout = timedelta(minutes=args.dist_timeout_minutes)
    ddp, ddp_rank, ddp_local_rank, ddp_world_size, device = compute_init(
        device_type, dist_timeout=dist_timeout
    )
        ...
        with autocast_ctx:
            ...
            if args.audit_dir:
                audit_file_path = os.path.join(
                    args.audit_dir,
                    f"{task_name.lower().replace('-', '_')}.jsonl",
                )
            acc = run_chat_eval(
                task_name,
                model,
