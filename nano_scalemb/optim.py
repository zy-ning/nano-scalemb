...
dynamo_config.recompile_limit = int(
    os.environ.get("NANOCHAT_DYNAMO_RECOMPILE_LIMIT", dynamo_config.recompile_limit)
)
dynamo_config.accumulated_recompile_limit = int(
    os.environ.get(
        "NANOCHAT_DYNAMO_ACCUMULATED_RECOMPILE_LIMIT",
        dynamo_config.accumulated_recompile_limit,
    )
)


def _async_handle(work):
    try:
...
Some of the changes in nano_scalemb implementation:
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        # Some params can be inactive on a given step and have grad=None; treat those
        # as zero gradients so the fused step can still run.
        grad_tensors = []
        for param in params:
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _reduce_adamw(self, group: dict, world_size: int) -> dict:
        ...
        for p in group["params"]:
            ...
            if grad is None:
                param_infos[p] = dict(
                    future=None, grad_slice=None, is_small=True, skip=True
                )
                continue
            if p.numel() < 1024:
                future = _async_handle(
                    dist.all_reduce(grad, op=dist.ReduceOp.AVG, async_op=True)
                )
                param_infos[p] = dict(
                    future=future, grad_slice=grad, is_small=True, skip=False
                )
                ...
                param_infos[p] = dict(
                    future=future, grad_slice=grad_slice, is_small=False, skip=False
                )
    ...
    def _reduce_muon(self, group: dict, world_size: int) -> dict:
        ...
        # Stack grads and zero-pad to padded_num_params. Some params may be inactive
        # on a given step and therefore have grad=None; treat them as zero grads.
        grad_tensors = []
        for param in params:
            if param.grad is None:
                grad_tensors.append(torch.zeros_like(param))
                ...
                grad_tensors.append(param.grad)
        grad_stack = torch.stack(grad_tensors)
    ...
    def _compute_muon(
        ...
        future = _async_handle(
            dist.all_gather_into_tensor(stacked_params, updated_params, async_op=True)
        )
        gather_list.append(
